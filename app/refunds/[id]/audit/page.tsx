...
import { ENTITY, getRefund } from "@/lib/refunds";
import { userName } from "@/lib/users";
import { when } from "../../_components/money";
import { Refusal } from "../../_components/refusal";

export const dynamic = "force-dynamic";

export default async function RefundAuditPage({
  params,
}: {
  ...
  const rows = auditFor(db, ENTITY, refund.id);
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">
          Audit trail <span className="font-mono text-lg text-zinc-600">{refund.id}</span>
        </h1>
        <p className="mt-1 text-sm text-zinc-600">
          Append-only. UPDATE and DELETE on <span className="font-mono">audit_log</span> are
